...
public class ContainerTeleporter extends Container
{
	public ContainerTeleporter(InventoryPlayer inventoryPlayer, IInventory inventory)
	{
		...
		{
			addSlotToContainer(new SlotCard(inventory, i, 8, 8 + 19 * i));
		}
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 9; j++)
			{
				addSlotToContainer(new Slot(inventoryPlayer, j + i * 9 + 9, 8 + j * 18, 165 + i * 18));
			}
		}

		for (int i = 0; i < 9; i++)
		{
			addSlotToContainer(new Slot(inventoryPlayer, i, 8 + i * 18, 223));
		}
	}
