...
public class Teleporter
{
	...
	public static class TeleportLocation
	{
		...
		{
			entity.worldObj.playSoundEffect(entity.posX, entity.posY, entity.posZ, Reference.MOD_ID.toLowerCase() + ":teleport", 0.1F, entity.worldObj.rand.nextFloat() * 0.1F + 0.9F);
	...
	private static Entity teleportEntity(Entity entity, TeleportLocation destination)
	{
		...
		World destinationWorld = CommonProxy.getMinecraftServer().worldServerForDimension(destination.dimension);
		...
		boolean interDimensional = startWorld.provider.getDimensionId() != destinationWorld.provider.getDimensionId();
		...
		if ((entity instanceof EntityPlayerMP) && interDimensional)
		{
			...
			player.playerNetServerHandler.sendPacket(new S07PacketRespawn(player.dimension, player.worldObj.getDifficulty(), destinationWorld.getWorldInfo().getTerrainType(), player.theItemInWorldManager.getGameType()));
		...
		if ((entity instanceof EntityPlayerMP))
		{
			...
			if (interDimensional)
			{
				player.mcServer.getConfigurationManager().transferEntityToWorld(player, 0, (WorldServer) player.worldObj, (WorldServer) destinationWorld);
			}
		...
		if (((entity instanceof EntityPlayerMP)) && interDimensional)
		{
			...
			FMLCommonHandler.instance().firePlayerChangedDimensionEvent(player, startWorld.provider.getDimensionId(), destinationWorld.provider.getDimensionId());
		}
		entity.setLocationAndAngles(destination.xCoord, destination.yCoord, destination.zCoord, destination.yaw, entity.rotationPitch);
